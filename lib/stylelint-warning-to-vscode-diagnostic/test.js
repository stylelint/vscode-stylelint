...
test('stylelintWarningToVscodeDiagnostic()', async (t) => {
	...
	t.throws(
		() => fn(),
		/Expected a stylelint warning.*line: <number>, colum: <number>, rule: <string>, severity: <string>, text: <string>/u,
		'should throw an error when it takes no arguments.',
	);

	t.throws(
		() => fn(Symbol('hi')),
		/^TypeError.*, but got Symbol\(hi\)\./u,
		'should throw an error when it takes a non-object argument.',
	);

	t.throws(
		() => fn({ line: Buffer.from('0') }),
		/^TypeError.*`line` property of a stylelint warning must be a number, but it was <Buffer 30>\./u,
		'should throw an error when `line` is a non-number value.',
	);

	t.throws(
		() => fn({ line: 10, column: new Set() }),
		/^TypeError.*`column` property of a stylelint warning must be a number, but it was Set(\(0\))? \{\}\./u,
		'should throw an error when `column` is a non-number value.',
	);

	t.throws(
		() => fn({ line: 10, column: 10, text: ['?'] }),
		/^TypeError.*`text` property of a stylelint warning must be a string, but it was \[ '\?' \] \(array\)\./u,
		'should throw an error when `text` is not a string.',
	);

	t.throws(
		() => fn({ line: 10, column: 10, text: '***' }),
		/^TypeError.*`severity` property of a stylelint warning must be either 'error' or 'warning'/u,
		'should throw an error when `severity` is not provided.',
	);

	t.throws(
		() => fn({ line: 10, column: 10, text: '***', severity: new Uint16Array(3) }),
		/^TypeError.*, but it was a non-string value Uint16Array(\(3\))? \[ 0, 0, 0 \]\./u,
		'should throw an error when `severity` is a non-string value.',
	);

	t.throws(
		() => fn({ line: 10, column: 10, text: '***', severity: 'errror' }),
		/^Error.*, but it was 'errror' \(string\)\./u,
