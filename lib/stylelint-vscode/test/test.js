...
test('stylelintVSCode()', async t => {
	...
	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document('this-file-is.empty', 'css', ''), {
...
a { color: #000 }
	...
	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document('fallback-to-lang-id.css', 'javascript', 'if(x) {color:#000000}'), {
				config: {
					rules: {
			...
			[],
			'should use the language ID as a fallback syntax when a warning is reported.'
		);
	})();

	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document(null, 'typescriptreact', 'styled.p`"`'), {
				config: {
					processors: [
	...
	(async () => {
		t.deepEqual(await stylelintVSCode(new Document('at', 'markdown', 'Yay\n```scss\n@\n```')), [
			{
				range: {
					start: {
						line: 2,
