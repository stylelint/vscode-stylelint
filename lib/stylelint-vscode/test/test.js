...
const createDocument = require('vscode-languageserver-types').TextDocument.create;
...
test('stylelintVSCode()', async t => {
	...
	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document('empty.css', 'css', ''), {
...
`), {
	...
	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document('noconfig.css', 'css', 'a{}')),
			[],
	...
	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document('css-in-js.js', 'javascript', `import glamorous from 'glamorous';
...
\`;`), {
	...
	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document('should-be-ignored.css', 'css', 'a { color: #000 }'), {
				config: {
					processors: [],
					rules: {
	...
	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document('f.i.x.t.u.r.e.tsx', 'typescriptreact', 'styled.p`"`'), {
				config: {
					processors: [
	...
	(async () => {
		t.deepEqual(await stylelintVSCode(new Document('unclosed.css', 'css', 'a{color:rgba(}')), [
			{
				range: {
					start: {
						line: 0,
	...
	(async () => {
		t.deepEqual(await stylelintVSCode(new Document('at.css', 'css', '@')), [
			{
				range: {
					start: {
						line: 0,
...
test('stylelintVSCode() with a configration file', async t => {
	...
	t.deepEqual(
		await stylelintVSCode(new Document(resolve(fixtureDir, 'has-config-file.tsx'), 'typescriptreact', `
