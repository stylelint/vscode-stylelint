...
a { color: #000 }
	...
	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document('unclosed.xml', 'xml', '<style>a{color:rgba(}</style>'))
			.then(r=>r.diagnostics),
			[
				{
					range: {
	...
	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document('at.xsl', 'xsl', '<style>@</style>'))
			.then(r=>r.diagnostics),
			[
				{
					range: {
...
test('stylelintVSCode() with autofix', async (t) => {
	...
	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document('no-rules.js', 'css', '"a"'), {
				config: {},
				fix: true
			})
			.then(r=>r.output),
