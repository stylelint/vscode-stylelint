...
`), {
	...
	(async () => {
		t.deepEqual(
			await stylelintVSCode(new Document(null, 'plaintext', '<style>a{}</style>'), {syntax: 'html'}),
			[],
...
a { color: #000 }
	...
	try {
		...
		t.ok(
			message.startsWith('`code`, `codeFilename`, `files` and `formatter` options are not supported '),
